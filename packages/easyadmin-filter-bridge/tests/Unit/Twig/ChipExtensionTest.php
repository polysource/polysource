 ...
 *
 * This extension exposes the chip-rendering helpers backed by
 * {@see ChipValueFormatter}: `polysource_chip_value` (since v0.1.4)
 * and `polysource_chip_operator` (since v1.1.2). The v0.1.2 stub for
 * `saved_views_dropdown` + the `polysource_saved_views_available()`
 * gate were removed when ownership of `saved_views_dropdown` moved to
...
{
    public function testGetFunctionsExposesOnlyChipHelpers(): void
    {
        ...
        self::assertSame(
            ['polysource_chip_value', 'polysource_chip_operator'],
            $names,
            'ChipExtension must expose exactly the chip-rendering '
            . 'helpers, both backed by ChipValueFormatter. If you are '
            . 'adding a helper outside the chip domain, put it in its '
            . 'own extension.'
        );
    }
